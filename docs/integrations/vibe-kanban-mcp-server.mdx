...
| `create_project` | Create a new project | `name`<br/>`repositories` (array, may be empty) | `description` | Created project with metadata |
...
| `create_task_and_start` | Create a task and immediately start a workspace session on it | `project_id`<br/>`title`<br/>`repos` | `description`<br/>`iteration`<br/>`executor`<br/>`variant` | Task ID and workspace ID |
| `get_task` | Get task details | `task_id` | None | Full task information |
| `get_task_status` | Aggregated task status: workspaces, latest execution, session IDs, pending approvals, PR status, diff stats | `task_id` | None | Task details with per-workspace status |
...
| `list_tags` | List all tags (reference as `@tag_name` in task descriptions) | None | None | List of tags with content |
...
| `list_branches` | List git branches (local and remote) of a repository | `repo_id` | None | Branch list with current/remote flags |
...
| `follow_up_session` | Send a follow-up instruction to a session (starts a new execution in the same workspace) | `session_id`<br/>`prompt` | `executor`<br/>`variant` | New execution process info |
| `queue_message` | Queue a message to be delivered when the session's current execution finishes | `session_id`<br/>`message` | `executor`<br/>`variant` | Queue confirmation |

Use `get_task_status` to find a workspace's `latest_session_id` for `follow_up_session` / `queue_message`.

### Approvals

| Tool | Purpose | Required Parameters | Optional Parameters | Returns |
|------|---------|-------------------|-------------------|---------|
| `list_approvals` | List pending tool-approval requests from running agents | None | None | List of pending approvals |
| `respond_to_approval` | Approve or deny a pending approval (`decision`: `approved` or `denied`) | `approval_id`<br/>`decision` | `reason` | Outcome of the response |
