...
pub struct ClientCapabilities {
    ...
    ///
    /// VENDOR PATCH (helios-kanban): not part of schema 0.9.1. kimi's ACP
    /// server checks `clientCapabilities.elicitation.form` to decide whether
    /// AskUserQuestion uses the native multi-question form
    /// (`elicitation/create`) instead of the single-question
    /// `session/request_permission` bridge. Remove once the upstream crate
    /// ships native elicitation support.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub elicitation: Option<Meta>,
    /// The _meta property is reserved by ACP to allow clients and agents to attach additional
    /// metadata to their interactions. Implementations MUST NOT make assumptions about values at
    /// these keys.
    ///
