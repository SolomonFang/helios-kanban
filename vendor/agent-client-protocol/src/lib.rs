...
impl Side for ClientSide {
    ...
    fn decode_request(method: &str, params: Option<&RawValue>) -> Result<AgentRequest> {
        ...
        match method {
            ...
            // VENDOR PATCH (helios-kanban): route kimi's `elicitation/create`
            // (an ACP method newer than this crate) through the extension
            // handler so `Client::ext_method` can answer it. Upstream routes
            // only `_`-prefixed methods there; without this arm the crate
            // replies method_not_found and kimi falls back to the
            // single-question request_permission bridge.
            m if m == "elicitation/create" => Ok(AgentRequest::ExtMethodRequest(ExtRequest::new(
                m,
